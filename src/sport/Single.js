...
export default function SingleSport() {
    ...
    const fetchData = () => {
        axios.get('http://localhost:8000/sport')
        .then((res) => {
            ...
            single?.map((item, index) => {
                return <div key={index} className='parent'>
                    ...
                    <div className="child">
                        ...
                        <div className='box'>
                            <button className='btn btn-primary' onClick={() => toggle(true)}>BUY NOW</button>
                            <button className='btn btn-light border'><i class="fa-solid fa-basket-shopping" onClick={() => sendBasket(item)}></i> ADD TO CART</button>
                        </div>
                    </div>
                </div>
            })
